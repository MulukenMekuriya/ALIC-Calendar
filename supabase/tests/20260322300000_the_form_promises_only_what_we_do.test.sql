...
-- 4. The form itself no longer offers a reissue the office cannot make, and
--    its hash still matches its text.
DO $a$
DECLARE _md UUID := 'a0eebc99-9c0b-4ef8-bb6d-6bb9bd380a11'; _t TEXT; _bad INT;
BEGIN
  SELECT s->>'text' INTO _t
  FROM church.consent_documents d, jsonb_array_elements(d.body) s
  WHERE d.organization_id=_md AND d.retired_at IS NULL AND s->>'key'='data_handling';

  IF _t ILIKE '%another copy at any time%' THEN
    RAISE EXCEPTION 'FAIL 4: the form still offers the office reissue';
  END IF;
  IF _t NOT ILIKE '%keep that email%' THEN
    RAISE EXCEPTION 'FAIL 4b: the form does not tell the parent to keep it';
  END IF;
  IF _t NOT ILIKE '%no address for you%' THEN
    RAISE EXCEPTION 'FAIL 4c: the form does not cover the family with no email';
  END IF;

  SELECT count(*) INTO _bad FROM church.consent_documents d
   WHERE d.body_sha256 <> church.consent_document_sha256(d.body);
  IF _bad > 0 THEN
    RAISE EXCEPTION 'FAIL 4d: % documents have a hash that no longer matches '
                    'their text - every signature taken after this would be '
                    'meaningless', _bad;
  END IF;
  RAISE NOTICE 'PASS 4 (form corrected, hash still true)';
END;
$a$;
